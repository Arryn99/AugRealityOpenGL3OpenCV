...
bool ScreenTexture::Initialize(OpenGLClass* OpenGL, char* filename, unsigned int textureUnit, bool wrap)
{
	bool result;
...
bool ScreenTexture::LoadTarga(OpenGLClass* OpenGL, char* filename, unsigned int textureUnit, bool wrap)
{
...
void ScreenTexture::createCameraTexture(OpenGLClass* OpenGL, unsigned int textureUnit) {
	// Set the unique texture unit in which to store the data.
	OpenGL->glActiveTexture(GL_TEXTURE0 + textureUnit);
...
void ScreenTexture::loadFrameIntoTexture(OpenGLClass* OpenGL, Mat &cameraFrame, unsigned int textureUnit) {
	...
	glTexImage2D(GL_TEXTURE_2D, 0, 3, cameraFrame.cols, cameraFrame.rows, 0, GL_RGB, GL_UNSIGNED_BYTE, cameraFrame.data);
